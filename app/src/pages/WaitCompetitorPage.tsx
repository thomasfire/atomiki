        ...
        <div className="w-full h-full flex content-center justify-center align-middle">
            <div className="grid h-min self-center">
                <div className="bg-white col-start-1 row-start-1 text-gray-800
                                        font-bold py-2 px-4 rounded h-min self-center m-3 justify-self-center">
                    {withUrl &&  <QRCodeSVG value={withUrl} />}
                </div>
                <div className="bg-white col-start-1 row-start-2 text-gray-800
                                        font-bold py-2 px-4 rounded h-min self-start m-3">
                    Share the Game ID to your competitor, so he can join the game
                </div>
                <input type="text" readOnly={true}
                       className={`py-2 px-4 rounded h-min self-center m-1 col-start-1 row-start-3 border-gray-200
                       border-solid border-2 focus-visible:border-gray-300 focus:border-gray-300 focus-visible:outline-hidden `}
                       value={withUrl || "Loading credentials..."}/>
                <CopyButton value={withUrl || ""} classes={"row-start-3"}/>
                <div className="bg-white col-start-1 row-start-4 text-gray-800
                                        font-bold py-2 px-4 rounded h-min self-start m-3">
